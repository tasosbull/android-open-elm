...
public class ElmCore {
    ...
    protected void Wait()
    {
        while ((comm.HasData()) && (!errorTimeout))
            gui.ProcMessages();
    }
    ...
    protected Globals.PROC_RES ProcessResponse(String cmd_sent, StringBuilder msg_received)
    {
        int i = 0;
        ...
        if (echo_on)
        {
            msg_received = new StringBuilder( msg_received.toString().substring(cmd_sent.length(), msg_received.toString().length() - cmd_sent.length()));
        ...
        if ((msg_received.length() >= 10) && (msg_received.substring(0, 10) == "<"))
        {
    ...
    private Globals.RESETSTATE Reset_get_reply_to_reset()
    {
        ...
        else if (errorTimeout) // if the timer timed out
        {
    ...
    private Globals.RESETSTATE Reset_get_reply_to_detect_protocol()
    {
        ...
        if (status == Globals.READ_RES.PROMPT) 
        {
            ...
            if (procRes == Globals.PROC_RES.ERR_NO_DATA || procRes == Globals.PROC_RES.UNABLE_TO_CONNECT)
    ...
    public String GetPidResponse(String pid, int pidBytes, boolean noError)
    {
        ...
        {
            cmd = "41" + pid;
            if (IsValidResponse(buf.toString(), vehicle_response.toString(), cmd))
            {
